...
const showToast = ({ type, title, description, duration = 3000, topOffset = 64 }: ToastParams): void => {
...
export const showAPIErrorToast = (error?: any) => {
    showToast({
        type: "error",
        description: error?.response?.data?.error || i18n.t("toast.apiErrorDefaultMessage")
    })
}
